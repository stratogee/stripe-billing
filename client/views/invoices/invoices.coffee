...
Template.invoices.rendered = ->
  ...
  if usr.billing?.customerId and not Session.get 'invoices.invoices.past'
    Session.set 'invoices.past.working', true
    Meteor.call 'getInvoices', Meteor.userId(), (error, response) ->
      Session.set 'invoices.past.working', false
      if error
        if error.error is 404
          Session.set 'invoices.invoices.past', null
        else
          Session.set 'invoices.error', t9n('Error getting past invoices')
      else
  ...
  if usr.billing and usr.billing.subscriptionId and not Session.get 'invoices.invoices.upcoming'
    ...
    Meteor.call 'getUpcomingInvoice', Meteor.userId(), (error, response) ->
      Session.set 'invoices.upcoming.working', false
      if error
        if error.error is 404
          Session.set 'invoices.invoices.upcoming', null
        else
          Session.set 'invoices.error', t9n('Error getting upcoming invoice')
      else
...
Template.invoices.helpers
  ...
  pricingPlan: ->
    ...
    if upcomingInvoice
      ...
      plan = sub.plan
      "#{inDollars(plan.amount)}/#{t9n(plan.interval)}"
...
Template.cancelSubscriptionModal.events
  'click .btn-confirm-cancel-subscription': (e) ->
    ...
    Meteor.call 'cancelSubscription', Meteor.userId(), Meteor.user().billing.customerId, (error, response) ->
      Session.set 'invoices.cancelingSubscription', false
      if error
...
Template._invoice.helpers
  lineItemDescription: ->
    if @type is 'subscription'
      "#{t9n("Subscription to")} #{@plan.name} (#{inDollars(@plan.amount)}/#{t9n(@plan.interval)})"
